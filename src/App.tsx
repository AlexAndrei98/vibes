...
const LogInPage: React.FC<{ setActiveComponent: React.Dispatch<React.SetStateAction<string>>, setLoggedInUser: React.Dispatch<React.SetStateAction<User | undefined>> }> = ({ setActiveComponent, setLoggedInUser }) => {
  const [enteredCode, setEnteredCode] = useState<number>();

  const handleLogin = () => {
    if (usersData.some(user => user.code === enteredCode)) {
      setActiveComponent('leaderboard');
      setLoggedInUser(usersData.find(user => user.code === enteredCode));
    } else {
...
const Leaderboard: React.FC<{ loggedInUser?: User, setLoggedInUser: React.Dispatch<React.SetStateAction<User | undefined>> }> = ({ loggedInUser, setLoggedInUser }) => {
  ...
  const handleClick = (direction: string, targetUser: User) => {
    ...
    //check if the user has already taken this specific action on this user for this week and if they have reset it and the score
  ...
  return (
    <div className='leaderboard'>
      ...
      {loggedInUser?.weeks.find(w => w.week === currentWeek)!.count < 0 &&
        <h2> You have no votes left</h2>}
      {loggedInUser?.weeks.find(w => w.week === currentWeek)!.count >= 0 &&
        <h2> You have {loggedInUser?.weeks.find(w => w.week === currentWeek)!.count} votes left</h2>}      {loggedInUser?.weeks.find(w => w.week === currentWeek)!.count > -1 &&
...
const HomePage: React.FC<{ setActiveComponent: React.Dispatch<React.SetStateAction<string>> }> = ({ setActiveComponent }) => {
  ...
  const handleClaim = () => {
    ...
    if (userIndex === -1) {
      ...
      usersData[userIndex].weeks.push({ week: currentWeek, count: 2 });
    }
  ...
  return (
    <Layout className="layout">
      ...
      <Content style={{ padding: '50px' }}>
        {activeComponent === 'login' && <LogInPage setActiveComponent={setActiveComponent} setLoggedInUser={setLoggedInUser} />}
        {activeComponent === 'leaderboard' && <Leaderboard loggedInUser={loggedInUser} setLoggedInUser={setLoggedInUser} />}
        {activeComponent === '' && <HomePage setActiveComponent={setActiveComponent} />}
      </Content>
    </Layout>
  );
}
